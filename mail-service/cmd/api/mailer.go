...
func (m *Mail) SendMail(msg Message) error {
	...
	msg.Datamap = data
	formatMessage, err := m.buildMessage(msg)
	if err != nil {
		log.Println("failed to build message", err)
		return err
	}
	plainMessage, err := m.buildPlainTextMessage(msg)
	if err != nil {
		log.Println("failed to plain template", err)
		return err
	}
...
func (m *Mail) buildMessage(msg Message) (string, error) {
	...
	if err != nil {
		log.Println("failed to generate css", err)
		return "", err
	}
...
func (m *Mail) inlineCss(message string) (string, error) {
	...
	if err != nil {
		log.Println("failed to execute premailer", err)
		return "", err
	}
	html, err := prem.Transform()
	if err != nil {
		log.Println("failed to transform to html", err)
		return "", err
	}
	return html, nil
}
